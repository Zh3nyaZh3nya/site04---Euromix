export const state = () => ({
  mainCatalogData: [
    {
      id: 1,
      factoriesData: [
        {
          id: 1,
          idRoute: new Date(),
          title: "Мобильные бетонные заводы Euromix Crocus",
          subtitle: "Мобильный бетонный завод EUROMIX CROCUS 10.500, производительность до 10 м³/ч, весовая система дозирования цемента и инертных заполнителей",
          img: "/main/catalogMain/img1.png",
          pageContent: {
            article: 1,
            title: "МОБИЛЬНЫЙ БЕТОННЫЙ ЗАВОД EUROMIX CROCUS (КРОКУС) 30/750.4 COMPACT 1",
            img: "/main/catalogMain/img1.png",
            sliderImg: [
              {
                id: 1,
                img: "/main/catalogMain/img3.png"
              },
              {
                id: 2,
                img: "/main/catalogMain/img2.png"
              },
              {
                id: 3,
                img: "/main/catalogMain/img4.png"
              },
              {
                id: 4,
                img: "/main/catalogMain/img5.png"
              },
            ],
            price: 32858,
            specifications: [
              {
                id: 1,
                name: "БетоносмесительEUROMIX",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            completeSets:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            desc:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            middleImg: [
              {
                id: 1,
                img: "/cardImg/image1.png",
              },
              {
                id: 2,
                img: "/cardImg/image1.png",
              },
            ],
            videoPresentation: [
              {
                id: 1,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 2,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 3,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 4,
                img: "/cardImg/image2.png",
                link: "",
              },
            ],
            count: 1,
          }
        },
        {
          id: 2,
          idRoute: new Date(),
          title: "Бетонные заводы серии Euromix Crocus Compact",
          subtitle: "Мобильный бетонный завод EUROMIX CROCUS 10.500, производительность до 10 м³/ч, весовая система дозирования цемента и инертных заполнителей",
          img: "/main/catalogMain/img2.png",
          pageContent: {
            article: 2,
            title: "Бетонные заводы серии Euromix Crocus Compact",
            img: "/main/catalogMain/img1.png",
            sliderImg: [
              {
                id: 1,
                img: "/main/catalogMain/img4.png"
              },
              {
                id: 2,
                img: "/main/catalogMain/img3.png"
              },
              {
                id: 3,
                img: "/main/catalogMain/img2.png"
              },
              {
                id: 4,
                img: "/main/catalogMain/img5.png"
              },
            ],
            price: 54354,
            specifications: [
              {
                id: 1,
                name: "БетоносмесительEUROMIX",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            completeSets:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            desc:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            middleImg: [
              {
                id: 1,
                img: "/cardImg/image1.png",
              },
              {
                id: 2,
                img: "/cardImg/image1.png",
              },
            ],
            videoPresentation: [
              {
                id: 1,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 2,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 3,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 4,
                img: "/cardImg/image2.png",
                link: "",
              },
            ],
            count: 1,
          }
        },
        {
          id: 3,
          idRoute: new Date(),
          title: "Бетонные заводы серии Euromix Crocus alfa (скип)",
          subtitle: "Мобильный бетонный завод EUROMIX CROCUS 10.500, производительность до 10 м³/ч, весовая система дозирования цемента и инертных заполнителей",
          img: "/main/catalogMain/img3.png",
          pageContent: {
            article: 3,
            title: "Бетонные заводы серии Euromix Crocus alfa (скип)",
            img: "/main/catalogMain/img1.png",
            sliderImg: [
              {
                id: 1,
                img: "/main/catalogMain/img6.png"
              },
              {
                id: 2,
                img: "/main/catalogMain/img7.png"
              },
              {
                id: 3,
                img: "/main/catalogMain/img8.png"
              },
              {
                id: 4,
                img: "/main/catalogMain/img5.png"
              },
            ],
            price: 13452,
            specifications: [
              {
                id: 1,
                name: "БетоносмесительEUROMIX",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            completeSets:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            desc:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            middleImg: [
              {
                id: 1,
                img: "/cardImg/image1.png",
              },
              {
                id: 2,
                img: "/cardImg/image1.png",
              },
            ],
            videoPresentation: [
              {
                id: 1,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 2,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 3,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 4,
                img: "/cardImg/image2.png",
                link: "",
              },
            ],
            count: 1,
          }
        },
        {
          id: 4,
          idRoute: new Date(),
          title: "Бетонные заводы серии Euromix Crocus alfa (лента)",
          subtitle: "Мобильный бетонный завод EUROMIX CROCUS 10.500, производительность до 10 м³/ч, весовая система дозирования цемента и инертных заполнителей",
          img: "/main/catalogMain/img4.png",
          pageContent: {
            article: 4,
            title: "Бетонные заводы серии Euromix Crocus alfa (лента)",
            img: "/main/catalogMain/img1.png",
            sliderImg: [
              {
                id: 1,
                img: "/main/catalogMain/img1.png"
              },
              {
                id: 2,
                img: "/main/catalogMain/img2.png"
              },
              {
                id: 3,
                img: "/main/catalogMain/img3.png"
              },
              {
                id: 4,
                img: "/main/catalogMain/img4.png"
              },
            ],
            price: 29000,
            specifications: [
              {
                id: 1,
                name: "БетоносмесительEUROMIX",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            completeSets:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            desc:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            middleImg: [
              {
                id: 1,
                img: "/cardImg/image1.png",
              },
              {
                id: 2,
                img: "/cardImg/image1.png",
              },
            ],
            videoPresentation: [
              {
                id: 1,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 2,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 3,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 4,
                img: "/cardImg/image2.png",
                link: "",
              },
            ],
            count: 1,
          }
        },
        {
          id: 5,
          idRoute: new Date(),
          title: "Автомиксер серии Euromix Rolltruck",
          subtitle: "Мобильный бетонный завод EUROMIX CROCUS 10.500, производительность до 10 м³/ч, весовая система дозирования цемента и инертных заполнителей",
          img: "/main/catalogMain/img5.png",
          pageContent: {
            article: 5,
            title: "Автомиксер серии Euromix Rolltruck",
            img: "/main/catalogMain/img1.png",
            sliderImg: [
              {
                id: 1,
                img: "/main/catalogMain/img4.png"
              },
              {
                id: 2,
                img: "/main/catalogMain/img5.png"
              },
              {
                id: 3,
                img: "/main/catalogMain/img6.png"
              },
              {
                id: 4,
                img: "/main/catalogMain/img7.png"
              },
            ],
            price: 65440,
            specifications: [
              {
                id: 1,
                name: "БетоносмесительEUROMIX",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            completeSets:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони (бетоносмеситель)",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            desc:  [
              {
                id: 1,
                name: "Бетоносмеситель",
                text: "600.800.2",
              },
              {
                id: 2,
                name: "Наличие брони",
                text: "Да",
              },
              {
                id: 3,
                name: "Бункера (кол-во х м3)",
                text: "2х15",
              },
              {
                id: 4,
                name: "Подача инертных материалов",
                text: "СКИП",
              },
              {
                id: 5,
                name: "Дозирование заполнителей, цемента",
                text: "ВЕСОВАЯ ",
              },
              {
                id: 6,
                name: "Дозирование воды",
                text: "ВЕСОВАЯ",
              },
              {
                id: 7,
                name: "Силоса цемента, т (дополнительно)",
                text: "от 10 - 120",
              },
              {
                id: 8,
                name: "Режим работы",
                text: "АВТОМАТ",
              },
              {
                id: 9,
                name: "Установленная мощность",
                text: " кВтдо 50",
              },
              {
                id: 10,
                name: "Масса не более",
                text: "кг11 000",
              },
              {
                id: 11,
                name: "ДхШхВ",
                text: "мм",
              },
            ],
            middleImg: [
              {
                id: 1,
                img: "/cardImg/image1.png",
              },
              {
                id: 2,
                img: "/cardImg/image1.png",
              },
            ],
            videoPresentation: [
              {
                id: 1,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 2,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 3,
                img: "/cardImg/image2.png",
                link: "",
              },
              {
                id: 4,
                img: "/cardImg/image2.png",
                link: "",
              },
            ],
            count: 1,
          }
        },
      ],
    },
    {
      id: 2,
      productsData: [
        {
          id: 1,
          idRoute: new Date(),
          title: "Бетоносмесители",
          img: "/main/catalogMain/img6.png",
        },
        {
          id: 2,
          idRoute: new Date(),
          title: "Силосы",
          img: "/main/catalogMain/img7.png",
        },
        {
          id: 3,
          idRoute: new Date(),
          title: "Дозирующие компоненты инертных материалов",
          img: "/main/catalogMain/img8.png",
        },
      ],
    },
    {
      id: 3,
      accessoriesData: [
        {
          id: 1,
          idRoute: new Date(),
          title: "Дозирующие компоненты инертных материалов",
          img: "/main/catalogMain/img1.png"
        }
      ]
    }
  ],
  selectData: null,
})

export const mutations = {
  SET_SELECT_DATA(state, data) {
    state.selectData = data;
  },
}

export const actions = {
  setSelectData({commit}, data) {
    commit('SET_SELECT_DATA', data)
  },
}

export const getters = {
  getSelectData: (state) => state.selectData,
};
